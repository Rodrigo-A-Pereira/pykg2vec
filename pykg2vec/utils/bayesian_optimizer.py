...
class BaysOptimizer:
    ...
    def __init__(self, args):
        ...
        if args.model_name.lower() in ["convkb", "proje_pointwise"]:
    ...
    def optimize(self):
        ...
        ################################## My code ########################################
    def optimize_mrr(self):
        trials = Trials()
        self._best_result = fmin(fn=self._get_imrr, space=self.search_space, trials=trials,
                                    algo=tpe.suggest, max_evals=self.max_evals)

        columns = list(self.search_space.keys())
        results = pd.DataFrame(columns=['iteration'] + columns + ['mrr'])
        ...
        path = self.config_local.path_result / self.model_name
        path.mkdir(parents=True, exist_ok=True)
        results.to_csv(str(path / "trials_mrr.csv"), index=False)
    ...
    ################################## My code ########################################
    def _get_imrr(self,params):
        # copy the hyperparameters to trainer config and hyperparameter set.
        for key, value in params.items():
            self.config_local.__dict__[key] = value
        self.config_local.__dict__['device'] = self.kge_args.device
        if self.model_name.lower() in ["conve"]:
            self.config_local.__dict__['neg_rate'] = 0
        ...
        mrr = self.trainer.tune_model_mrr()
        inv_mrr= -mrr

        # Key name must be 'loss', given it is a requiremente from the hyperopt library that offerst the fmin function used
        return {'loss': inv_mrr, 'status': STATUS_OK}
    ################################## \My code ########################################
